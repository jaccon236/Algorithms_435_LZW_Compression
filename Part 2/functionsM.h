/*
    ...
    Purpose: function header file for Algorithms Project 2 (Part 2)
*/

// This code is derived in parts from LZW@RosettaCode for UA CS435

#ifndef FUNCTIONSM_H_INCLUDED
#define FUNCTIONSM_H_INCLUDED
...
void writeCompressedFile(vector<int>, string);
        ...
        it != uncompressed.end(); ++it) 
   {
      ...
      else 
      {
         *result++ = dictionary[w];
         ...
         // Add wc to the dictionary. Max size is 16 bits or 65536
...
string decompress(Iterator begin, Iterator end) 
{
   ...
   for ( ; begin != end; begin++) 
   {
      ...
      // Add w + entry[0] to the dictionary. Max size is 16 bits or 65536
      if (dictionary.size() < 65536)
      {
