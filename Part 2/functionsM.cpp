/*
    ...
    Purpose: functions file for Algorithms Project 2 (Part 2)
*/
...
// This function is refactored and extracted from binaryIODemo() provided in the assignment
void writeCompressedFile(vector<int> compressed, string fileName)
{
    string bcode= "";    
    string p;

    // Start with a minimum 10 bits
    int bits = 10;
    int count = 256;

    for (vector<int>::iterator it = compressed.begin() ; it != compressed.end(); ++it) 
    {
        // If count exceeds the available room with the current number of bits
        // and if bits is not currently already 16, increment bits
        if (count++ >= pow(2, bits) && bits != 16)
        {
            bits++;
        }
    ...
    // Add ".lzw2" to compressed files
    if (fileName.find(".lzw2") == string::npos)
    {
        fileName += ".lzw2";
    }
...
// Separates the compressed data in a string into binary codes. The length of 
// the binary codes are determined via the size of the file. The while loop 
// continues until the end of the file is reached by index. Each time it loops
// the iterations of the for loop increase by a power of 2. This is because 
// for the first 256 characters of fileData, the minimum bits, 10, are used
// to encode the compressed file. After each 2^(bits-1) characters of fileData
// the total number of bits used to encode the data (the binary code length)
// increases by one until a maximum of 16. Once the end of the file is reached
// the vector<int> containing all of the codes are returned. 
...
vector<int> separateCodes(const string &fileData, int bits)
{
    ...
    while (true)
    {
        ...
        // Wait until iterations = 2^(bits-1) to increase bits but do not exceed 16 bits
        if (power > 9 && bits != 16)
        {
            bits++;
        } 
